...
namespace IMAGECONVERT
{
    ...
    public class ImageConvert : TerrariaPlugin
    {
        ...
        private void image(CommandArgs args)
        {
            ...
            {
                args.Player.SendErrorMessage("Invalid syntax! proper syntax: /image <load/paste>");
                return;
            }
            switch (args.Parameters[0])
            {
                case "list":
                    #region list
                    List<string> files = new List<string>(Directory.GetFiles(savepath));
                    IEnumerable<string> result = from s in files where checkextension(s) select s;
                    //list
                    #endregion list
                    return;
                case "load":
        ...
        public bool DrawImage(CommandArgs args)
        {
                try
                {
                    for (int i = 0; i < Image[args.Player.Index].Width; i++)
                    {
                        for (int j = 0; j < Image[args.Player.Index].Height; j++)
                        {
                            System.Drawing.Color pixel = Image[args.Player.Index].GetPixel(i, j);
                            Color c = new Color(pixel.R, pixel.G, pixel.B);
                            Main.tile[args.Player.TileX + i, args.Player.TileY + j] = FindTileByColor(c);
                        }
                    }
                    Image[args.Player.Index] = null;
                    return true;
                }
                catch (Exception ex) { Log.ConsoleError(ex.ToString()); return false; }
        }
